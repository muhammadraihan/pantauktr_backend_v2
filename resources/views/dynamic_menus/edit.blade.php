...
<div class="row">
    <div class="col-xl-6">
        <div id="panel-1" class="panel">
            <div class="panel-hdr">
            <h2>Edit <span class="fw-300"><i>{{$dynamic_menu->uuid}}</i></span></h2>
            ...
            <div class="panel-container show">
                <div class="panel-content">
                    ...
                    {!! Form::open(['route' => ['dynamic_menu.update',$dynamic_menu->uuid],'method' => 'PUT','class' => 'needs-validation','enctype' => 'multipart/form-data','novalidate']) !!}
                    ...
                    <div class="form-group col-md-4 mb-3">
                        <img id="image-preview" src="{{$dynamic_menu->icon}}" class="shadow-2 img-thumbnail"
                            alt="">
                    </div>
                    <div class="form-group col-sm-6 col-xl-4">
                        {{ Form::label('judul','Judul',['class' => 'required form-label'])}}
                        {{ Form::text('judul',$dynamic_menu->judul,['placeholder' => 'Judul','class' => 'form-control '.($errors->has('judul') ? 'is-invalid':''),'required', 'autocomplete' => 'off'])}}
                        @if ($errors->has('judul'))
                    ...
                    <div class="form-group col-sm-6 col-xl-4">
                        ...
                        <select class="js-bg-color custom-select select2" name="status">
                                @if ($dynamic_menu->status == 1)
                                <option value="{{$dynamic_menu->status}}">Aktif</option>
                                @else
                                <option value="{{$dynamic_menu->status}}">Non Aktif</option>
                                @endif
